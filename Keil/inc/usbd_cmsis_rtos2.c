...
int32_t USBD_Delay (uint32_t millisec) {
  static uint32_t kernel_tick_freq = 0U;

  if (kernel_tick_freq == 0U) {
    kernel_tick_freq = osKernelGetTickFreq ();
  }

  if (kernel_tick_freq == 1000U) {
    return (osDelay (millisec));
  } else {
    return (osDelay ((uint32_t)((uint64_t)millisec * kernel_tick_freq) / 1000U));
  }
}
...
int32_t USBD_TimerStart (void *timer_hndl, uint32_t millisec) {
  ...
  if (kernel_tick_freq == 1000U) {
    return (osTimerStart ((osTimerId_t)timer_hndl, millisec));
  } else {
    return (osTimerStart ((osTimerId_t)timer_hndl, (uint32_t)((uint64_t)millisec * kernel_tick_freq) / 1000U));
  }
}
...
uint32_t USBD_ThreadFlagsWait (uint32_t millisec) {
  ...
  if ((kernel_tick_freq == 1000U) || (millisec == 0U)) {
    res = osThreadFlagsWait (0xFFFFU, osFlagsWaitAny, millisec);
  } else {
    res = osThreadFlagsWait (0xFFFFU, osFlagsWaitAny, (uint32_t)(((uint64_t)millisec * kernel_tick_freq) / 1000U));
  }
...
int32_t USBD_SemaphoreAcquire (void *semaphore_hndl, uint32_t millisec) {
  ...
  if ((kernel_tick_freq == 1000U) || (millisec == 0U)) {
    return (osSemaphoreAcquire ((osSemaphoreId_t)semaphore_hndl, millisec));
  } else {
    return (osSemaphoreAcquire ((osSemaphoreId_t)semaphore_hndl, (uint32_t)((uint64_t)millisec * kernel_tick_freq) / 1000U));
  }
}
